...
Working IP Address space for me is 10.255.255.X 255.255.255.0. I will be using .162, .163, and .164 for these VMs.
...
1. Network Adapter (bottom right corner) > Open Network and Sharing Center > Change Adapter Settings > Ethernet 0 > Properties
...
2. Expand DNS > Win-VCSKKNLVACD (should change depending on your Win Server hostname is, that's autogenerated during install)
3. Right click Forward Lookup Zones → New Zone 
4. Next > Primary Zone > Zone Name
    Voip.local (if you have an alternate local domain name, use it) 
5. Create a new file with this file name:
    Voip.local (if you have an alternate local domain name, use it)
...
7. Now right click on the right hand side → New Host (A or AAAA)....
    Insert name “esxi”, this will autofill “esxi.voip.local”.
8. Insert IP address of “esxi” node if/when installed
9. Add host
10. Repeat the above for "vcsa"
...
2. Attempt to access via Web browser using the [ESXi Host's FQDN](https://esxi.voip.local/)
    Verify that you can log in and browse around the ESXi Web Interace
...
5. Storage path
    ...
    This may change for you depending on where you store your VMs
6. Press next
...
8. Press next
10. Networking Configuration (Fill these in from top down)
    Ipv4
        Static
            X.x.x.x (IP address)
...
14. <pw> 
15. Miscellaneous
16. If it says "false", leave it "false". 
    In hindsight, editing this post a few years down the road, I don't know what I was thinking with steps 11 - 16, or why there's so little detail... but I do know that I thought this was enough.
17. Networking Properties
    Vcsa
    Voip.local
18. Click Import

Please wait for some time while the following takes place
...
Access the VM via HTTPs using https://vcsa.voip.local:5480/ in a Web Browser. It will prompt to login using root credentials (the password set when deploying the OVF template) and  you will be greeted with an indication that the setup is in progress.
...
VM will complete set up and provide link to access. Click on the Flash based weblink. This will lead you to the following URL. Bookmark it. In fact, bookmark both of the below (Do note that the URL may be different depending on your chosen domain and vm hostnames):
